...
import { X, Copy, Check, ExternalLink, Smartphone, Wifi, RefreshCw, Hash, Share2 } from 'lucide-react';
...
export default function RoomInfoModal({ onClose }: RoomInfoModalProps) {
  ...
  // Compute canonical URL that works everywhere
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm">
        <motion.div
          ...
          className="w-full max-w-md bg-neutral-950 border border-saffron-500/40 rounded-3xl p-6 shadow-[0_0_50px_rgba(255,107,26,0.3)] relative text-center text-white space-y-4"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          {/* Title */}
          <div>
            <h2 className="font-display text-2xl font-bold text-white mb-1 flex items-center justify-center gap-2">
              <Hash size={24} className="text-saffron-400" /> Mobile Buzzer Room
            </h2>
            <p className="text-neutral-400 text-xs">
              Share the 6-digit code or link with team members to hit the live buzzer!
            </p>
          </div>

          {/* Prominent 6-Digit Room Code Card */}
          <div className="bg-gradient-to-r from-saffron-950/90 via-neutral-900 to-saffron-950/90 border-2 border-saffron-500/60 rounded-2xl p-5 shadow-lg flex flex-col items-center justify-center gap-2 relative overflow-hidden">
            ...
            <div className="flex items-center justify-center gap-1.5 sm:gap-2 my-2">
              {roomId.split('').map((digit, idx) => (
                <div
                  key={idx}
                  className="w-10 h-13 sm:w-12 sm:h-14 bg-black/90 border-2 border-saffron-400/80 rounded-xl flex items-center justify-center font-mono font-black text-2xl sm:text-3xl text-saffron-300 shadow-md"
                >
                  {digit}
                </div>
              ))}
            ...
            <div className="flex items-center gap-2 mt-1">
              <button
                onClick={copyCode}
                className="text-xs bg-saffron-500/20 hover:bg-saffron-500/30 text-saffron-300 px-3.5 py-1.5 rounded-xl font-mono font-bold flex items-center gap-1.5 border border-saffron-500/30 transition-colors"
              >
              ...
              <button
                ...
                className="text-xs bg-neutral-800 hover:bg-neutral-700 text-neutral-300 px-3 py-1.5 rounded-xl flex items-center gap-1.5 border border-neutral-700 transition-colors"
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="bg-neutral-900/90 border border-neutral-800 p-3 rounded-2xl">
              <span className="text-[10px] font-bold text-saffron-400 block uppercase mb-0.5">Step 1</span>
              <span className="text-xs text-white font-medium block">Open Share Link or App</span>
            </div>
            <div className="bg-neutral-900/90 border border-neutral-800 p-3 rounded-2xl">
              <span className="text-[10px] font-bold text-saffron-400 block uppercase mb-0.5">Step 2</span>
              <span className="text-xs text-white font-medium block">Enter Code {formattedCode}</span>
            </div>
          </div>
          ...
          <div className="space-y-2.5 pt-1">
            <div className="flex items-center gap-2 bg-neutral-900 border border-neutral-800 rounded-xl p-2 pl-3">
              <Share2 size={16} className="text-saffron-400 shrink-0" />
